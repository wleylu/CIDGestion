            ...
            <div class="p-6 bg-white border-b border-gray-200">
                <div lass="align-middle inline-block min-w-full shadow
                    ...
                    <div class="right-10 col-md-1 py-2">
                        <a href="{{ route('produit.create') }}"  class="text-white bg-orange-700 hover:bg-indigo-500 focus:ring-4
                         ...
                         <table id="example" class=" py-1 table table-striped table-bordered table-responsive" style="width:100%">
                                ...
                                <tbody>
                                    @foreach ($produits as $produit)
                                        <tr>
                                            <td>{{ $produit ->codeProd }}</td>
                                            <td>{{ $produit->produit }}</td>
                                            <td>{{ $produit->cid_commission->libelle }}</td>
                                            <td>{{ $produit->cid_periode->libelle}}</td>
                                            <td>{{ $produit->taux}}</td>
                                            <td>{{ $produit->commission}}</td>
                                            <td>


                                                <a type="button" href="{{ route('produit.edit',['id'=>$produit->id]) }}" class="text-primary px-2" >
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                ...
                                                <a type="button" href="{{ route('produit.delete',['id'=>$produit->id]) }}" class="text-danger">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
