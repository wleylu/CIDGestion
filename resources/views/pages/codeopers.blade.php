            ...
            <div class="p-6 bg-white border-b border-gray-200">
                <div lass="align-middle inline-block min-w-full shadow
                    ...
                    <div class="alert-danger text text-2xl text-center  text-bold text-primary">
                        LISTE DES CODES OPERATIONS PARAMETRES
                    </div>

                    <div class="right-10 col-md-1 py-2">
                        <a href="{{ route('codeoper.create') }}"  class="text-white bg-orange-700 hover:bg-indigo-500 focus:ring-4
                    ...
                    @endif
                         ...
                         <table id="example" class=" py-1 table table-striped table-bordered table-responsive" style="width:100%">
                                ...
                                <tbody>
                                    @foreach ($codeopers as $oper)
                                        <tr>
                                            <td>{{ $oper ->oper }}</td>
                                            <td>{{ $oper ->libelle }}</td>
                                            <td>{{ $oper ->acteur == 'I' ? 'Compte interne':'Compte client' }}</td>
                                            <td>{{ $oper ->taux }}</td>
                                            <td>{{ $oper ->mntCom }}</td>
                                            <td>{{ $oper ->compteOper }}</td>
                                            <td>{{ $oper ->compteCom }}</td>
                                            <td>{{ $oper->description }}</td>
                                            <td>
                                                @if ($oper->cid_commission_id)
                                                 {{ $oper->cid_commission->libelle }}
                                                @endif
                                            </td>
                                            <td>
                                                <a type="button" href="{{ route('codeoper.edit',['id'=>$oper->id]) }}" class="text-primary px-2" >
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                ...
                                                <a type="button" href="{{ route('codeoper.delete',['id'=>$oper->id]) }}" class="text-danger">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
